...
class Mensagem(models.Model):
    ...
    def __str__(self):
        return self.texto[:50] # Mostra os primeiros 50 caracteres no admin
```
...
def pagina_principal(request):
    ...
    if request.method == 'POST':
        ...
        if texto_da_mensagem:
            Mensagem.objects.create(texto=texto_da_mensagem)
        return redirect('pagina_principal') # Redireciona para evitar reenvio

    # Se for GET (acesso normal à página), busca todas as mensagens
    mensagens = Mensagem.objects.all().order_by('-data_criacao') # Mais recentes primeiro
...
Baixe a extençao "Djangle" e "Django Template"
